...
export default function TutorReviewModal({
  bookingId,
  parentName,
  subject,
  existingReview,
      ...
      <div className="neu-card w-full max-w-lg space-y-5 bg-white p-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-black text-[#0F172A]">
                {existingReview ? "Edit Your Feedback" : "Rate This Student"}
              </h2>
            </div>
            <p className="mt-0.5 text-xs font-semibold text-slate-500">
              {parentName}&apos;s student · {subject}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {isLocked && (
          <div className="rounded-2xl border-2 border-[#FCA5A5] bg-[#FEF2F2] px-4 py-3 text-xs font-semibold text-red-700">
            ⏰ The 48-hour edit window has closed. This feedback can no longer be changed.
          </div>
        )}
        ...
        {!isLocked && (
          <form action={formAction} className="space-y-5">
            ...
            {/* Tutor reviews student using same DB fields, remapped */}
            <StarRating
              name="teachingRating"
              label="Student Behaviour & Engagement"
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                id="tutor-review-comment"
                name="comment"
                rows={3}
                defaultValue={existingReview?.comment ?? ""}
                placeholder="Share feedback about the session…"
        ...
        {isLocked && (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              {[
                { label: "Student", value: existingReview?.teachingRating },
