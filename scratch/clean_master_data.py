...
def clean_location(val):
    ...
    s = s.replace('_', ' ')
    # Clean duplicates like "East Delhi, Delhi" -> "East Delhi"
    s = re.sub(r'\bDelhi\s*,\s*Delhi\b', 'Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bEast Delhi\s*,\s*Delhi\b', 'East Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bWest Delhi\s*,\s*Delhi\b', 'West Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bNorth Delhi\s*,\s*Delhi\b', 'North Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bSouth Delhi\s*,\s*Delhi\b', 'South Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bCentral Delhi\s*,\s*Delhi\b', 'Central Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bNorth West Delhi\s*,\s*Delhi\b', 'North West Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bSouth West Delhi\s*,\s*Delhi\b', 'South West Delhi', s, flags=re.IGNORECASE)
    s = re.sub(r'\bNorth East Delhi\s*,\s*Delhi\b', 'North East Delhi', s, flags=re.IGNORECASE)
    ...
    s = s.strip(' ,;.')
    return s if s and s.lower() not in ['-', 'na', 'none'] else None

def clean_subject_item(subj):
    if not subj: return ""
    s = str(subj).strip().strip('"\'`*')
    s = re.sub(r'\s+', ' ', s)
    if s.lower() in ['-', '--', 'na', 'none', 'null']: return ""
    return s
...
for r in tutor_ncr_sheets['sheet_1'][1:]:
    ...
    loc = clean_location(area) or clean_location(city) or "Delhi NCR"
    ...
    subj_list = [clean_subject_item(s) for s in re.split(r'[,;/|\n&]+', subjects) if clean_subject_item(s)]
...
for r in rohit_sheets['sheet_1'][1:]:
    ...
    loc = clean_location(clean_text(r[7] if len(r)>7 else ""))
    ...
    for r in exp_rows:
        ...
        if phone:
            add_lead(phone, {
                ...
                "location": clean_location(addr) or "Delhi NCR",
                "pincode": None,
                "fullAddress": addr,
                "subjects": [],
                "classes": [],
...
for r in shl_sheets['sheet_1']:
    ...
    area = clean_location(clean_text(r[4] if len(r)>4 else ""))
    gender_pref = clean_gender(r[5] if len(r)>5 else "")
    extra_addr = clean_text(r[6] if len(r)>6 else "")

    if phone:
        loc = clean_location(area) or clean_location(extra_addr) or "Delhi NCR"
        ...
        cleaned_sub = clean_subject_item(name_or_sub)
        ...
        add_lead(phone, {
            ...
            "pincode": None,
            "fullAddress": loc,
            "subjects": [cleaned_sub] if (is_subject and cleaned_sub) else [],
            "classes": classes_list,
            "qualification": None,
            "experienceYears": None,
            "gender": gender_pref,
...
for r in syntax_sheets['sheet_1'][1:]:
    ...
    phone = clean_phone(r[1] if len(r)>1 else "")
    loc = clean_location(clean_text(r[2] if len(r)>2 else ""))
    if phone:
        add_lead(phone, {
            "leadType": "INSTITUTE",
...
for idx, text in enumerate(female_doc):
    ...
    if phone:
        ...
        cleaned_sub = clean_subject_item(subj)
        add_lead(phone, {
            "leadType": "TUTOR",
            "name": name,
            "phone": phone,
            ...
            "pincode": None,
            "fullAddress": None,
            "subjects": [cleaned_sub] if cleaned_sub else [],
            "classes": [],
            "qualification": None,
            "experienceYears": None,
            "gender": "Female",
...
with open(csv_file_path, "w", newline="", encoding="utf-8") as f:
    ...
    for lead in all_leads_list:
        row = dict(lead)
        # Semicolon separated for subjects to avoid comma collisions in CSV
...
print(f"Master files regenerated with clean locations and subjects! Total: {len(all_leads_list)}")
