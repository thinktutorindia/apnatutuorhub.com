...
async function run() {
  ...
  const res = await fetch('https://verified.aquasms.com/pyramid-get-app-list', {
    headers: { 'Cookie': cookieHeader, 'X-Requested-With': 'XMLHttpRequest' },
  });
  console.log('/pyramid-get-app-list status:', res.status);
  console.log('App list text:', await res.text());
}
