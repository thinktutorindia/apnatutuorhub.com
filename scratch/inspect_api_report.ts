...
async function run() {
  ...
  const reportPage = await fetch('https://verified.aquasms.com/api-report-master', {
    headers: { 'Cookie': cookieHeader },
  });
  const html = await reportPage.text();
  const selectMatch = html.match(/<select[^>]*name="swabanumber"[^>]*>[\s\S]*?<\/select>/i);
  console.log('swabanumber select:');
  console.log(selectMatch ? selectMatch[0] : 'not found');
}
