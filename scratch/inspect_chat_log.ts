...
async function run() {
  ...
  const res = await fetch('https://verified.aquasms.com/chat-log', {
    headers: { 'Cookie': cookieHeader },
  });
  console.log('/chat-log status:', res.status);
  const html = await res.text();
  const clean = html.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  console.log('Chat log snippet:');
  console.log(clean.slice(0, 1500));

  // Find scripts related to chat log
  const scripts = html.match(/<script[^>]*src="([^"]+)"/gi) || [];
  console.log('\nChat log scripts:', scripts.filter(s => s.includes('chat')));
}
