...
const sampleText = `26/08/26, 11:09 am - Messages and calls are end-to-end encrypted. Only people in this chat can read, listen to, or share them. *Learn more*
...
async function test() {
  ...
  const res = await parseWhatsAppDump(sampleText);
  const elapsed = Date.now() - startTime;
  console.log("Parsed " + res.leads.length + " leads in " + elapsed + "ms");
  console.log("Sample leads:", JSON.stringify(res.leads.slice(0, 5), null, 2));
}
